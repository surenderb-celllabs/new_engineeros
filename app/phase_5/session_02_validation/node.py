...
from app.phase_5.session_02_validation import STEP_PATH, State, ResponseType
...
class UserStoryOptimizerRequirement(BaseNode):
    def __init__(self, agent_tools: List):
        user_story = load_yaml("app/phase_5/user_stories.yaml")
        prompt_path = Path(STEP_PATH) / "prompts/user_story_validation.md"
        prompt_data = load_file(str(prompt_path)).format(user_stories=user_story)
        
        super().__init__(agent_tools, True, prompt_data, model=Model.Groq.gpt_oss_20b)
        
        self.node_logger.debug(user_story)
    ...
    def __call__(self, state: State) -> State:
        try:
            ...
            # invoke_data = state["messages"]
            # if len(state["messages"]) < 3:
            #     invoke_data = self.prompt + state["messages"]
...
class UserStoryOptimizerDocument(BaseNode):
    def __init__(self, agent_tools: List):
        prompt_path = Path(STEP_PATH) / "prompts/user_story_doc.md"
        prompt_data = load_file(str(prompt_path))
        super().__init__(
            agent_tools, False, prompt_data,
            model=Model.Groq.gpt_oss_20b
        )
        self.user_stories = load_yaml("app/phase_5/user_stories.yaml")
