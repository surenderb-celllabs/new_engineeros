...
from app.phase_5.session_01 import STEP_PATH, State, ResponseType
...
class ProductRequirement(BaseNode):
    def __init__(self, agent_tools: List):
        prompt_path = Path(STEP_PATH) / "prompts/problem.md"
        prompt_data = load_file(str(prompt_path))
        super().__init__(agent_tools, True, prompt_data, Model.Groq.gpt_oss_20b)

    def __call__(self, state: State) -> State:
        try:
            self._log_section_header("Problem Analysis")
...
class ProblemDocument(BaseNode):
    def __init__(self, agent_tools: List):
        prompt_path = Path(STEP_PATH) / "prompts/problem_doc.md"
        prompt_data = load_file(str(prompt_path))
        super().__init__(
    ...
    def __call__(self, state: State) -> State:
        try:
            self._log_section_header("Problem Doc Generator")
