...
from app_1.phase_5.session_01 import STEP_PATH, State, ResponseType
...
class ProblemStatements(BaseNode):
    def __init__(self, agent_tools: List):
        prompt_path = Path(STEP_PATH) / "prompts/business_problem.md"
        prompt_data = load_file(str(prompt_path))
        super().__init__(agent_tools, True, prompt_data, Model.ChatOllama.gpt_oss_20b)

    def __call__(self, state: State) -> State:
        try:
            self._log_section_header(f"Problem Analysis > {len(state['messages'])}")
            ...
            if convo_end:
                self._log_section_header("Document")
                self.node_logger.info(resp_json["po_message"])

            return {
                "messages": [AIMessage(content=json.dumps(resp_json["po_message"]))],
